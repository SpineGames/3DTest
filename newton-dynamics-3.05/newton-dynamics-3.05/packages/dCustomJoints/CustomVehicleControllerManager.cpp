...
#define VEHICLE_PSD_DAMP_TOL                    dFloat(1.0e-3f)
...
{
	m_count = points;
	_ASSERTE (points < (sizeof(m_nodes)/sizeof (m_nodes[0])));
	...
	,m_currentGear(m_newtralGear)
{
	...
	_ASSERTE (gearCount < (m_maxGears - 2));
...
void CustomVehicleController::EngineComponent::InitEngineTorqueCurve (dFloat vehicleSpeedKPH, dFloat idleTorque, dFloat idleTorqueRPM, dFloat peakTorque, dFloat peakTorqueRPM, dFloat peakHorsePower, dFloat peakHorsePowerRPM, dFloat redLineTorque, dFloat redLineTorqueRPM)
{
	...
	_ASSERTE (idleTorqueRPM > 0.0f);
	_ASSERTE (idleTorqueRPM < peakTorqueRPM);
	_ASSERTE (peakTorqueRPM < peakHorsePowerRPM);
	_ASSERTE (peakHorsePowerRPM < redLineTorqueRPM);

	_ASSERTE (idleTorque > 0.0f);
	_ASSERTE (idleTorque < peakTorque);
	_ASSERTE (peakTorque > torqueAtPeakPower);
	_ASSERTE (torqueAtPeakPower > redLineTorque);
	_ASSERTE (redLineTorque > 0.0f);

	_ASSERTE (peakTorque * peakTorqueRPM < peakHorsePower);
...
{
	_ASSERTE (topSpeedMPS >= 0.0f);
	_ASSERTE (topSpeedMPS < 100.0f);
	...
	for (int i = 0; i < m_contactCount; i ++) {
		...
		if (mag2 > 1.0e-3f) {
			...
			if (dAbs (u) > 1.0f) {
				...
				_ASSERTE (sideSlipAngle >= 0.0f);
				_ASSERTE (sideSlipAngle <= 3.141592);
			}
			...
			_ASSERTE (normalizedLateralForce >= 0.0f);
			_ASSERTE (normalizedLateralForce <= 1.0f);

			dFloat normalizedLongitudinalForce = longitudinalSlipRationCurve.GetValue (longitudinalSlipRatio);
			_ASSERTE (normalizedLongitudinalForce >= 0.0f);
			_ASSERTE (normalizedLongitudinalForce <= 1.0f);
...
void CustomVehicleController::TireBodyState::UpdateDynamicInputs(dFloat timestep)
{
	...
	if (m_contactJoint.m_contactCount) {
		...
		_ASSERTE (distance >= 0.0f);
		_ASSERTE (distance <= m_suspensionlenght);
		if (distance <= 1.0e-3f) {
...
void CustomVehicleController::Init (NewtonCollision* const chassisShape, const dMatrix& vehicleFrame, dFloat mass, const dVector& gravityVector)
{
	...
	if (shapeType == SERIALIZE_ID_COMPOUND) {
		...
		_ASSERTE ((shapeType == SERIALIZE_ID_CONVEXHULL) || (shapeType == SERIALIZE_ID_BOX));
...
int CustomVehicleController::GetActiveJoints(VehicleJoint** const jointArray)
{
	...
	for (TireList::dListNode* node = m_tireList.GetFirst(); node; node = node->GetNext()) {
		...
		jointCount ++;
		_ASSERTE (jointCount < VEHICLE_CONTROLLER_MAX_JOINTS);
	}
...
int CustomVehicleController::BuildJacobianMatrix (int jointCount, VehicleJoint** const jointArray, dFloat timestep, VehicleJoint::JacobianPair* const jacobianArray, VehicleJoint::JacobianColum* const jacobianColumnArray)
{
	...
	for (int j = 0; j < jointCount; j ++) {
		...
		for (int i = 0; i < dofCount; i ++) {
			...
			rowCount ++;
			_ASSERTE (rowCount < VEHICLE_CONTROLLER_MAX_JACOBIANS_PAIRS);
		}
...
void CustomVehicleController::CalculateReactionsForces (int jointCount, VehicleJoint** const jointArray, dFloat timestepSrc, VehicleJoint::JacobianPair* const jacobianArray, VehicleJoint::JacobianColum* const jacobianColumnArray)
{
	...
	for (dList<BodyState*>::dListNode* stateNode = m_stateList.GetFirst(); stateNode; stateNode = stateNode->GetNext()) {
		...
		stateIndex ++;
		_ASSERTE (stateIndex < sizeof (stateVeloc)/sizeof (stateVeloc[0]));
	}
