...
// FIXME Does window still exist?
    ...
    case WM_CREATE:
	  ...
	  // the standard window API only recognizes the joystick and four buttons
	  joySetCapture((HWND)hwnd, JOYSTICKID1, NULL, FALSE);
	  //joySetCapture((HWND)hwnd, JOYSTICKID2, NULL, FALSE);
...
/*
	case  MM_JOY2MOVE:
		#ifdef WIN32
		{
			...
			//UINT wZpos;                 
			//UINT wButtons;              
			event.win_x = info.wXpos; 
			event.win_y = info.wYpos; 
			event.code = info.wButtons; 
			window->handle(this,FXSEL(event.type,0),&event);
		}
		#endif
		break; 
*/
