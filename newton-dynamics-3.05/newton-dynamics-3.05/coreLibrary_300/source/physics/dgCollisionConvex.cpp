...
class dgCollisionConvex::dgMinkHull: public dgDownHeap<dgMinkFace *, dgFloat32>  
{
	...
	dgMinkHull(dgCollisionParamProxy& proxy)
		...
		,m_freeFace(NULL)
	{
		_ASSERTE (m_proxy->m_referenceCollision->IsType(dgCollision::dgCollisionConvexShape_RTTI));
		_ASSERTE (m_proxy->m_floatingCollision->IsType (dgCollision::dgCollisionConvexShape_RTTI));
		_ASSERTE (m_contactJoint && m_contactJoint->GetId() == dgConstraint::m_contactConstraint);
	}
	...
	DG_INLINE dgMinkFace* NewFace()
	{
		...
		if (m_freeFace) {
			...
			m_faceIndex ++;
			_ASSERTE (m_faceIndex < DG_CONVEX_MINK_MAX_FACES);
		}
	...
	void SupportVertex (const dgVector& dir, dgInt32 vertexIndex)
	{
		...
		_ASSERTE (dgAbsf(dir1 % dir1 - dgFloat32 (1.0f)) < dgFloat32 (1.0e-3f));
	...
	static void ReduceDegeneratedTriangle (dgVector& diff0, dgVector& sum0, dgVector& diff1, dgVector& sum1, const dgVector& diff2, const dgVector& sum2)
	{
		...
		if ((e10 % e10) < dgFloat32 (1.0e-12f)) {
			...
			diff1 = diff2;
						...
						_ASSERTE (((e20 % e20) < dgFloat32 (1.0e-8f)) || ((e21 % e21) < dgFloat32 (1.0e-8f)));
			#endif
		}
	}
	...
	static dgVector ReduceTetrahedrum (dgInt32& indexOut, dgVector* const tetraDiff, dgVector* const tetraSum, dgInt32* const shapeFaceIndex)
	{
		...
		for (dgInt32 i = 0; i < 4; i ++) {
			...
			_ASSERTE (dgAbsf (area) > dgFloat32 (0.0f));
	...
	static dgVector ReduceTetrahedrumLarge (dgInt32& indexOut, dgVector* const tetraDiff, dgVector* const tetraSum, dgInt32* const shapeFaceIndex)
	{
		...
		for (dgInt32 i = 0; i < 4; i ++) {
			...
			_ASSERTE (fabs (area) > dgFloat32 (0.0f));
	...
	void CalculateContactFromFeacture(dgInt32 featureType)
	{
		...
		switch (featureType)
		{
			...
			case 2:
			{
				...
				_ASSERTE (alpha0 <= dgFloat32 (1.01f));
				_ASSERTE (alpha0 >= dgFloat32 (-0.01f));
			...
			default:
			{
				...
				_ASSERTE (alphaDen > dgFloat32 (0.0f));

				alphaDen = dgFloat32 (1.0f / alphaDen);
				alphas[0] *= alphaDen;
				alphas[1] *= alphaDen;
				alphas[2] *= alphaDen;
				//_ASSERTE (alphas[0] >= dgFloat32 (-1.0e-2f));
				//_ASSERTE (alphas[1] >= dgFloat32 (-1.0e-2f));
				//_ASSERTE (alphas[2] >= dgFloat32 (-1.0e-2f));
		...
		//dir = normal;
		_ASSERTE (dgAbsf (m_normal % m_normal - dgFloat32 (1.0f)) < dgFloat32 (1.0e-4f)) ;
	...
	void CalculateClosestPoints ()
	{
		...
		_ASSERTE (collConvexInstance->m_childShape == m_otherShape);
		_ASSERTE (collConicConvexInstance->m_childShape == m_myShape);
		_ASSERTE (collConicConvexInstance->IsType (dgCollision::dgCollisionConvexShape_RTTI));
		...
		_ASSERTE ((simplexPointCount > 0) && (simplexPointCount <= 3));
	...
	dgInt32 CalculateIntersectingPlane (dgInt32 count)
	{
		...
		if (count == 1) {
			...
			_ASSERTE ((err % err) > dgFloat32 (0.0f));
			count = 2;

		}
		if (count == 2) {
			dgVector e0 (m_hullDiff[1] - m_hullDiff[0]);
			_ASSERTE ((e0 % e0) > dgFloat32 (0.0f));
			...
			_ASSERTE (maxVolume > dgFloat32 (0.0f));
			count ++;
		}
...
//}
				...
				while (stackIndex) {
					...
					if (!face->m_mark && (face->m_plane.Evalue(p) > dgFloat32(0.0f))) { 
							...
							for (dgInt32 i = 0; i < deletedCount; i ++) {
								_ASSERTE (m_deletedFaceList[i] != face);
							}
						#endif

						m_deletedFaceList[deletedCount] = face;
						deletedCount ++;
						_ASSERTE (deletedCount < sizeof (m_deletedFaceList)/sizeof (m_deletedFaceList[0]));
						face->m_mark = 1;
						...
						for (dgInt32 i = 0; i < 3; i ++) {
							...
							if (!twinFace->m_mark) {
								...
								stackIndex ++;
								_ASSERTE (stackIndex < sizeof (m_faceStack)/sizeof (m_faceStack[0]));
							}
						}
					}
				}
				...
				for (dgInt32 i = 0; i < deletedCount; i ++) {
					...
					for (dgInt32 j1 = 0; j1 < 3; j1 ++) {
						...
						if (!twinFace->m_mark) {
							...
							newCount ++;
							_ASSERTE (newCount < sizeof (m_coneFaceList)/sizeof (m_coneFaceList[0]));
						}
						j0 = j1;
					}
				}
				...
				m_vertexIndex ++;
				_ASSERTE (m_vertexIndex < sizeof (m_hullDiff)/sizeof (m_hullDiff[0]));
...
{
	_ASSERTE (m_rtti | dgCollisionConvexShape_RTTI);
	m_edgeCount= 0;
	m_vertexCount = 0;
	m_vertex = NULL;
	m_vertexToEdgeMap = NULL;
...
/*
	...
	if (m_vertexCount < DG_MAX_SIMD_VERTEX_LIMIT) {
		...
		_ASSERTE (!((dgUnsigned64(m_simdVertexSOA)) & (DG_VECTOR_AVX_SIZE - 1)));
...
//	}
//	_ASSERTE (m_supportVertexStarQuadrant[4] == GetSupportEdge (m_multiResDir[0].Scale (-1.0f)));
//	_ASSERTE (m_supportVertexStarQuadrant[5] == GetSupportEdge (m_multiResDir[1].Scale (-1.0f)));
//	_ASSERTE (m_supportVertexStarQuadrant[6] == GetSupportEdge (m_multiResDir[2].Scale (-1.0f)));
//	_ASSERTE (m_supportVertexStarQuadrant[7] == GetSupportEdge (m_multiResDir[3].Scale (-1.0f)));
...
{
	_ASSERTE (dgAbsf (normal % normal - dgFloat32 (1.0f)) < dgFloat32 (1.0e-3f));
...
bool dgCollisionConvex::PolygonOBBTest (const dgFloat32* const polygon, dgInt32 strideInBytes, const dgInt32* const indexArray, dgInt32 indexCount, const dgPolygonMeshDesc& descInfo) const
{
	...
	if (faceSize > (dgFloat32 (3.0f) * dgMax(m_boxSize.m_x, m_boxSize.m_y, m_boxSize.m_z))) {
		...
		do {
			...
			_ASSERTE ((diff % diff) > dgFloat32 (0.0f));
...
bool dgCollisionConvex::SeparetingVectorTest (dgCollisionParamProxy& proxy) const
{
	...
	_ASSERTE (collConicConvexInstance->m_childShape == this);
	_ASSERTE (collConicConvexInstance->IsType (dgCollision::dgCollisionConvexShape_RTTI));
	...
	do {	
		...
		_ASSERTE (dgAbsf(dir % dir - dgFloat32 (1.0f)) < dgFloat32 (1.0e-4f));
		iter ++;
...
dgInt32 dgCollisionConvex::ConvexPolygonToLineIntersection (const dgVector& normal, dgInt32 count1, dgVector* const shape1, dgInt32 count2, dgVector* const shape2, dgVector* const contactOut, dgVector* const mem) const
{
	...
	if (count2 == 2) {
		...
		for (dgInt32 i1 = 0; i1 < count1; i1 ++) {
			...
			count = 0;
			i0 = i1;
			//_ASSERTE (output < &pool[sizeof (pool)/sizeof (pool[0])]);
		}
...
dgInt32 dgCollisionConvex::ConvexPolygonsIntersection (const dgVector& normal, dgInt32 count1, dgVector* const shape1, dgInt32 count2, dgVector* const shape2, dgVector* const contactOut, dgInt32 maxContacts) const
{
	...
	if (count2 <= 2) {
		...
		_ASSERTE ((2 * (count1 + count2)) < dgInt32 (sizeof (subdivision) / sizeof (subdivision[0])));
		for (dgInt32 i0 = 0; i0 < count2; i0 ++) {
...
dgInt32 dgCollisionConvex::CalculateContactsGeneric (const dgVector& point, const dgVector& normal, dgCollisionParamProxy& proxy, dgVector* const contactsOut) const
{
	...
	_ASSERTE (myInstance->m_childShape == this);
	const dgCollisionConvex* const otherShape = (dgCollisionConvex*)convexInstance->m_childShape;
	_ASSERTE (otherShape->IsType(dgCollisionConvexShape_RTTI));
