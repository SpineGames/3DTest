	...
	void CreatePairsJobs (dgBroadPhase::dgNode* const rootNode)
	{
		...
		while (stack && (m_pairsCount < dgInt32 (sizeof (m_pairs) / (2 * sizeof (m_pairs[0]))))) {
			...
			while (stack >= stackDpeth) {
				...
				m_pairsCount ++;
				_ASSERTE (m_pairsCount < (sizeof (m_pairs) / (2 * sizeof (m_pairs[0]))));
			}
...
{
	_ASSERTE ((body0->GetInvMass().m_w != dgFloat32 (0.0f)) || (body1->GetInvMass().m_w != dgFloat32 (0.0f)) || (body0->IsRTTIType(dgBody::m_kinematicBodyRTTI)) || (body1->IsRTTIType(dgBody::m_kinematicBodyRTTI)));
...
//	if (body0->GetCollision()->GetCollisionMode() & body1->GetCollision()->GetCollisionMode()) {
		...
		if ((body0->IsRTTIType(dgBody::m_kinematicBodyRTTI)) || (body0->GetInvMass().m_w != dgFloat32 (0.0f))) {
			...
			_ASSERTE ((body1->GetInvMass().m_w != dgFloat32 (0.0f)) || (body1->IsRTTIType(dgBody::m_kinematicBodyRTTI)));
...
{
//	_ASSERTE (m_world == descriptor->m_world);
	...
	for ( ; node; ) {
		...
		if (body->IsRTTIType(dgBody::m_dynamicBodyRTTI)) {
			...
			_ASSERTE (body->IsRTTIType(dgBody::m_kinematicBodyRTTI));
...
dgInt32 dgBroadPhase::ConvexCast (dgCollisionInstance* const shape, const dgMatrix& matrix, const dgVector& target, dgFloat32& timeToImpact, OnRayPrecastAction prefilter, void* const userData, dgConvexCastReturnInfo* const info, dgInt32 maxContacts, dgInt32 threadIndex) const
{
	...
	dgInt32* attributeA[DG_CONVEX_CAST_POOLSIZE];
	dgInt32* attributeB[DG_CONVEX_CAST_POOLSIZE];
