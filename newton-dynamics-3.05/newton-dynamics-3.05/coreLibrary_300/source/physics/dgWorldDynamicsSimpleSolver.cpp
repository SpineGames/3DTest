						...
						if (body0->m_continueCollisionMode | body1->m_continueCollisionMode) {
							...
							if (t < timeToImpact) {
								if (t == dgFloat32 (0.0f)) {
									...
									_ASSERTE (dgAbsf (normal % normal - dgFloat32 (1.0f)) < dgFloat32 (1.0e-2f));
									...
									if (speed > dgFloat32 (0.0f)) {
										...
										if (speed < dgFloat32 (2.0f)) {
											...
											_ASSERTE (distance > dgFloat32 (0.0f));
	...
	if (jointCount) {
		...
		for (dgInt32 k = 0; k < jointCount; k ++) {
			...
			for (dgInt32 i = 0; i < count; i ++) {
				...
				_ASSERTE (row->m_jointFeebackForce);
...
void dgWorldDynamicUpdate::CalculateSimpleBodyReactionsForces (const dgIsland* const island, dgInt32 rowStart, dgInt32 threadIndex, dgFloat32 timestep, dgFloat32 maxAccNorm) const
{
	...
	for (dgInt32 i = 0; i < count; i ++) {
		...
		_ASSERTE ((frictionIndex < 0) || ((frictionIndex >= 0) && (matrixRow[frictionIndex].m_force >= dgFloat32 (0.0f))));
		...
		for (dgInt32 passes = 0; (passes < maxPasses) && (accNorm > maxAccNorm); passes ++) {
			...
			for (dgInt32 curJoint = 0; curJoint < jointCount; curJoint ++) {
				...
				for (dgInt32 k = 0; k < rowsCount; k ++) {
					...
					dgInt32 frictionIndex = row->m_normalForceIndex;
					_ASSERTE (((frictionIndex < 0) && (normalForce[frictionIndex] == dgFloat32 (1.0f))) || ((frictionIndex >= 0) && (normalForce[frictionIndex] >= dgFloat32 (0.0f))));
...
//				              row->m_JMinv.m_jacobianM1.m_angular.CompProduct (y1.m_angular));
	...
	for (dgInt32 i = 0; i < jointCount; i ++) {
		...
		for (dgInt32 k = 0; k < count; k ++) {
			...
			dgInt32 frictionIndex = row->m_normalForceIndex;
			_ASSERTE ((frictionIndex < 0) || ((frictionIndex >= 0) && (rowBase[frictionIndex].m_force >= dgFloat32 (0.0f))));
