...
dgInt32 dgWorld::ClosestCompoundPoint (dgBody* const compoundConvexA, dgBody* const collisionB, dgTriplex& contactA, dgTriplex& contactB, dgTriplex& normalAB, dgInt32 threadIndex) const
{
	...
	_ASSERTE (instance->IsType(dgCollision::dgCollisionCompound_RTTI));
...
void dgWorld::PopulateContacts (dgCollidingPairCollector::dgPair* const pair, dgFloat32 timestep, dgInt32 threadIndex)
{
	...
	_ASSERTE (contact->m_broadphaseLru == GetBroadPhase()->m_lru);
...
void dgWorld::ProcessDeformableContacts (dgCollidingPairCollector::dgPair* const pair, dgFloat32 timestep, dgInt32 threadIndex)
{
	...
	_ASSERTE (0);
	_ASSERTE (contact->GetBody1() == pair->m_body0);
	_ASSERTE (contact->GetBody0() == pair->m_body1);
...
{
	_ASSERTE (contact && (contact->GetId() == dgConstraint::m_contactConstraint));
...
void dgWorld::DeformableContacts (dgCollidingPairCollector::dgPair* const pair, dgCollisionParamProxy& proxy) const
{
	...
	*/
	...
	_ASSERTE (deformable->IsType(dgCollision::dgCollisionDeformableMesh_RTTI));
...
/*
	...
	if (otherBody->m_collision->IsType (dgCollision::dgCollisionConvexShape_RTTI)) {
		...
		_ASSERTE (convexBody->m_collision->IsType (dgCollision::dgCollisionConvexShape_RTTI));
		_ASSERTE (otherBody->m_collision->IsType (dgCollision::dgCollisionConvexShape_RTTI));
		...
		_ASSERTE (constraint->m_body0->m_collision->IsType (dgCollision::dgCollisionConvexShape_RTTI));
		_ASSERTE (convexBody->m_collision->IsType (dgCollision::dgCollisionConvexShape_RTTI));
...
void dgWorld::SceneContactsSimd (const dgCollisionScene::dgProxy& sceneProxy, dgCollidingPairCollector::dgPair* const pair, dgCollisionParamProxy& proxy) const
{
	...
	_ASSERTE (pair->m_contact->GetBody1()->GetCollision()->IsType(dgCollision::dgCollisionScene_RTTI));
...
void dgWorld::SceneContacts (dgCollidingPairCollector::dgPair* const pair, dgCollisionParamProxy& proxy) const
{
	...
	_ASSERTE (sceneInstance->IsType(dgCollision::dgCollisionScene_RTTI));
	_ASSERTE (!otherInstance->IsType(dgCollision::dgCollisionScene_RTTI));
...
//	dgFloat32& retTimeStep, dgTriplex* const points, dgTriplex* const normals, dgFloat32* const penetration, dgInt32 maxSize, dgInt32 threadIndex)
	...
	dgInt32** const attibuteA, dgInt32** const attibuteB, dgInt32 maxContacts, dgInt32 threadIndex)
{
	_ASSERTE (0);
	return 0;
}
	...
	dgInt32** const attibuteA, dgInt32** const attibuteB, dgInt32 maxContacts, dgInt32 threadIndex)
{
	...
	if (count) {
		...
		for (dgInt32 i = 0; i < count; i ++) {
			...
			attibuteA[i] = (dgInt32*)contacts[i].m_shapeId0;
			attibuteB[i] = (dgInt32*)contacts[i].m_shapeId1;
		}
	} 
	return count;
}
...
//	_ASSERTE (0);
	_ASSERTE (contactJoint);
//	_ASSERTE (!proxy.m_continueCollision);
	_ASSERTE (collision1->GetCollisionPrimityType() != m_nullCollision);
	_ASSERTE (collision2->GetCollisionPrimityType() != m_nullCollision);
	_ASSERTE (proxy.m_floatingCollision->IsType (dgCollision::dgCollisionConvexShape_RTTI));
...
dgInt32 dgWorld::CalculateConvexToNonConvexContacts (dgCollisionParamProxy& proxy) const
{
	...
	if (data.m_faceCount) {
		...
		for (dgInt32 i = 0; i < count; i ++) {
			_ASSERTE ((dgAbsf(contactOut[i].m_normal % contactOut[i].m_normal) - dgFloat32 (1.0f)) < dgFloat32 (1.0e-4f));
