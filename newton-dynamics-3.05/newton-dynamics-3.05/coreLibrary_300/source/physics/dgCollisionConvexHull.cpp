			...
			if (dist > maxProj) {
				...
				if (box.m_leftBox > 0) {
					...
					if (rightDist >= leftDist) {
						...
						stack ++;
						_ASSERTE (stack < sizeof (distPool)/sizeof (distPool[0]));
					...
					for (dgInt32 i = 0; i < box.m_vertexCount; i ++) {
						...
						_ASSERTE (p.m_x >= box.m_box[0].m_x);
						_ASSERTE (p.m_x <= box.m_box[1].m_x);
						_ASSERTE (p.m_y >= box.m_box[0].m_y);
						_ASSERTE (p.m_y <= box.m_box[1].m_y);
						_ASSERTE (p.m_z >= box.m_box[0].m_z);
						_ASSERTE (p.m_z <= box.m_box[1].m_z);
