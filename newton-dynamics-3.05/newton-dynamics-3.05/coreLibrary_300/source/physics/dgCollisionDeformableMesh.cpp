...
class dgCollisionDeformableMesh::dgDeformationRegion
{
	...
	void UpdateVelocities(const dgCollisionDeformableMesh::dgParticle& particles, dgFloat32 timestep, dgFloat32 stiffness)
	{
		...
		m_rotSeed = S;
			...
			_ASSERTE (dgAbsf (xx[0][0] - dgFloat32 (1.0f)) < dgFloat32 (1.0e-3f));
			_ASSERTE (dgAbsf (xx[1][1] - dgFloat32 (1.0f)) < dgFloat32 (1.0e-3f));
			_ASSERTE (dgAbsf (xx[2][2] - dgFloat32 (1.0f)) < dgFloat32 (1.0e-3f));
			_ASSERTE (dgAbsf (xx[0][1]) < dgFloat32 (1.0e-3f));
			_ASSERTE (dgAbsf (xx[0][2]) < dgFloat32 (1.0e-3f));
			_ASSERTE (dgAbsf (xx[1][0]) < dgFloat32 (1.0e-3f));
			_ASSERTE (dgAbsf (xx[1][2]) < dgFloat32 (1.0e-3f));
			_ASSERTE (dgAbsf (xx[2][0]) < dgFloat32 (1.0e-3f));
			_ASSERTE (dgAbsf (xx[2][1]) < dgFloat32 (1.0e-3f));
		#endif
#endif
...
/*
	...
	if (m_rootNode) {
		_ASSERTE (IsType (dgCollision::dgCollisionDeformableMesh_RTTI));
		...
		if (side0 <= dgFloat32 (0.0f)) {
			...
			if (side1 >= dgFloat32 (0.0f)) {
				...
				if (inside) {
					...
					_ASSERTE (dgAbsf(den) > dgFloat32 (1.0e-6f));
