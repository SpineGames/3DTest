	...
	if (source.m_root) {
		...
		while (stack) {
				...
				if (!sourceNode->m_parent) {
					...
					if (parent->m_parent) {
						if (sourceNode->m_parent->m_left == sourceNode) {
							...
							_ASSERTE (sourceNode->m_parent->m_right == sourceNode);
			...
			if (sourceNode->m_left) {
				...
				stack ++;
				_ASSERTE (stack < DG_COMPOUND_STACK_DEPTH);
			}
...
//	,m_preCollisionFilter(NULL)
{
	_ASSERTE (m_rtti | dgCollisionCompound_RTTI);
...
void dgCollisionCompound::MassProperties ()
{
	...
	_ASSERTE (volume_ > dgFloat32 (0.0f));
	...
	if (!m_root) {
		...
		if (!sibling->m_parent) {
			...
			if (parent->m_left == sibling) {
				...
				_ASSERTE (parent->m_right == sibling); 
				dgNodeBase* const node = new (m_world->GetAllocator()) dgNodeBase (sibling, newNode);
				parent->m_right = node;
				node->m_parent = parent;
			}
...
void dgCollisionCompound::RemoveCollision (dgNodeBase* const treeNode)
{
	if (!treeNode->m_parent) {
		...
		if (treeNode->m_parent->m_left == treeNode) {
			...
			_ASSERTE (treeNode->m_parent->m_left == treeNode);
...
{
	_ASSERTE (node->GetInfo());
	_ASSERTE (node->GetInfo()->GetShape());
...
dgInt32 dgCollisionCompound::CalculateContacts (dgCollidingPairCollector::dgPair* const pair, dgCollisionParamProxy& proxy, dgInt32 useSimd) const
{
	...
	if (m_root) {
		...
		if (body1->m_collision->IsType (dgCollision::dgCollisionConvexShape_RTTI)) {
			...
			_ASSERTE (body1->m_collision->IsType (dgCollision::dgCollisionUserMesh_RTTI));
...
void dgCollisionCompound::CalculateCollisionTreeArea(dgNodePairs& pairOut, const dgCollisionBVH* const collisionTree, const void* const treeNode) const
{
	...
	_ASSERTE (pairOut.m_treeNodeArea > dgFloat32 (0.0f));
}
...
dgInt32 dgCollisionCompound::CalculateContactsToSingle (dgCollidingPairCollector::dgPair* const pair, dgCollisionParamProxy& proxy, dgInt32 useSimd) const
{
	...
	_ASSERTE (compoundInstance->GetChildShape() == this);
				...
				if (me->m_type == m_leaf) {
					...
					if (subShape->GetCollisionMode()) {
						...
						if (processContacts) {
							...
							for (dgInt32 i = 0; i < count; i ++) {
								_ASSERTE (contacts[contactCount + i].m_collision0 == &childInstance);
								//_ASSERTE (contacts[contactCount + i].m_collision1 == &otherChildInstance);
...
dgInt32 dgCollisionCompound::CalculateContactsToCollisionTree (dgCollidingPairCollector::dgPair* const pair, dgCollisionParamProxy& proxy, dgInt32 useSimd) const
{
	...
	_ASSERTE (compoundInstance->GetChildShape() == this);
	_ASSERTE (treeCollisionInstance->IsType (dgCollision::dgCollisionBVH_RTTI));
	...
	if (useSimd) {
		...
		while (stack) {
			...
			_ASSERTE (dgAbsf(area - stackEntry->m_treeNodeArea) < dgFloat32 (1.0e-1f));
#endif
...
dgInt32 dgCollisionCompound::CalculateContactsToHeightField (dgCollidingPairCollector::dgPair* const pair, dgCollisionParamProxy& proxy, dgInt32 useSimd) const
{
	...
	_ASSERTE (compoundInstance->GetChildShape() == this);
	_ASSERTE (terrainInstance->IsType (dgCollision::dgCollisionHeightField_RTTI));
...
dgInt32 dgCollisionCompound::CalculateContactsUserDefinedCollision (dgCollidingPairCollector::dgPair* const pair, dgCollisionParamProxy& proxy, dgInt32 useSimd) const
{
	...
	_ASSERTE (compoundInstance->GetChildShape() == this);
	_ASSERTE (userMeshInstance->IsType (dgCollision::dgCollisionUserMesh_RTTI));
