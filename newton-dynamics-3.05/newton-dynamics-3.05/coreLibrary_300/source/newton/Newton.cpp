...
void NewtonSetMinimumFrameRate(const NewtonWorld* const newtonWorld, dFloat frameRate)
{
	...
	TRACE_FUNCTION(__FUNCTION__);
	_ASSERTE (dgFloat32(1.0f) / dgFloat32 (DG_MAX_TIMESTEP) < dgFloat32(1.0f) / dgFloat32 (DG_MIN_TIMESTEP));
...
int NewtonCollisionCollide(const NewtonWorld* const newtonWorld, int maxSize,
						   ...
						   int** const attributeA, int** const attributeB, int threadIndex)
{
	TRACE_FUNCTION(__FUNCTION__);
	Newton* const world = (Newton *)newtonWorld;
...
dFloat NewtonCollisionRayCast(const NewtonCollision* const collisionPtr, const dFloat* const p0, const dFloat* const p1, dFloat* const normal, int** const attribute)
{
	dgCollisionInstance* const collision = (dgCollisionInstance*) collisionPtr;
	...
	if (t >= dFloat (0.0f) && t <= dFloat (dgFloat32(1.0f))) {
		attribute[0] = (int*)contact.m_shapeId0;
...
void NewtonCollisionGetInfo(const NewtonCollision* const collision, NewtonCollisionInfoRecord* const collisionInfo)
{
	...
	_ASSERTE ( dgInt32 (sizeof (dgCollisionInfo)) <= dgInt32 (sizeof (NewtonCollisionInfoRecord)));
