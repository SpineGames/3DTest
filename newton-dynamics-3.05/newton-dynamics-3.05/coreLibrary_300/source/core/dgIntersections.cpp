...
//	return  dgPointToTriangleDistance____ (point, p0, p1, p2);
	...
	_ASSERTE (fabsf (error < dgFloat32 (1.0e-5f)));
	_ASSERTE (faceNormalNormal > dgFloat32 (0.0f));
#endif
		...
		for (dgInt32 i = 0; i < m_count; i ++) {
			...
			if (alpha < dgFloat32 (0.0f)) {
				...
				if ((plane0.Evalue(p0) >= dgFloat32 (0.0f)) && (plane3.Evalue(p0) >= dgFloat32 (0.0f))) {
					...
					_ASSERTE ((plane2.Evalue(p0) <= dgFloat32 (0.0f)) && (plane3.Evalue(p0) <= dgFloat32 (0.0f)));
